...
import com.sphere.user.dto.response.ErrorResponse.ValidationErrorDetail;
...
/**
 * Production-grade Centralized Exception Handler for user-service.
 */
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleMaxUploadSize(MaxUploadSizeExceededException ex, HttpServletRequest request) {
        String traceId = getOrCreateTraceId(request);
        log.warn("[traceId={}] Max upload size exceeded at {}", traceId, request.getRequestURI());
        return respond(HttpStatus.PAYLOAD_TOO_LARGE, ErrorType.BadRequest.name(), "Uploaded file exceeds the maximum allowed size limit", request, traceId, null);
    }
    ...
    public ResponseEntity<ErrorResponse> handleDataIntegrity(DataIntegrityViolationException ex, HttpServletRequest request) {
        ...
        return respond(HttpStatus.CONFLICT, ErrorType.Conflict.name(), "Database integrity violation (e.g. duplicate username/email or relationship conflict)", request, traceId, null);
    }

    @ExceptionHandler(BadCredentialsException.class)
    public ResponseEntity<ErrorResponse> handleBadCredentials(BadCredentialsException ex, HttpServletRequest request) {
        String traceId = getOrCreateTraceId(request);
        return respond(HttpStatus.BAD_REQUEST, ErrorType.BadRequest.name(), "Incorrect email or password", request, traceId, null);
    }
